class SessionMessagesService
  ...
  def player_unavailable(unavailable_character)
    ...
    Message.create!(user: @dm, session: @session, content: message_dm, message_type: "dm_approval")
    message_players = "#{unavailable_character.campaign_character.user.nickname} can not make #{@session.date}"
    @players.each {|player| Message.create!(user: player,  session: @session, content: message_players, message_type: "player_notifcation")}
  end

  def get_dm_approval
    message = "All players have confirmed #{@session.date}. Do you approve this session?"
    Message.create!(user: @dm, session: @session, content: message, message_type: "dm_approval")
  end
  ...
  def session_confirm_or_reject(status)
    ...
    @players.each {|player| Message.create!(user: player, session: @session, content: message, message_type: "player_notification") }
  end
